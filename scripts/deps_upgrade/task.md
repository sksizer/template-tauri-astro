...
Run `pnpm update --latest` at the root and in `src-astro/`, but exclude every `@tauri-apps/*` package so they can be bumped atomically as one unit in step 4.
...
# In src-astro/
cd src-astro
cp package.json /tmp/src-astro-pkg.json.bak
pnpm update --latest 2>&1
...
- If `LATEST_MAJOR > CURRENT_MAJOR`, upgrade the entire Tauri set to `LATEST_MAJOR` in ONE logical change:
  ...
  - `src-astro/package.json`: set `@tauri-apps/api`, `@tauri-apps/plugin-opener`, and any other `@tauri-apps/*` to `^LATEST_MAJOR`.
  ...
  - Run `pnpm install` at root and in `src-astro/`, and `cargo update` in `src-tauri/`.
...
- Body: four sections —
  - **JS packages upgraded** (grouped by root / src-astro, with `old → new` version pairs)
